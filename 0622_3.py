...
df = pd.read_csv(file_path, encoding='cp949')    # DataFrame으로 만들어주기: Pandas 이용

df_clean = df[['일자', '통합 분류1']].dropna()   # 결측지 제거를 위함: dropna()

pivot_df = df_clean.groupby(['일자', '통합 분류1']).size().unstack(fill_value=0)  # groupby: ~ 별 ~ 구할 때 사용!
# 날짜별 통합 분류1의 값이 같은 것들을 모아 수를 세어서 통합 분류1의 값을 컬럼으로 해서 값을 넣어 표를 만들어라
# unstack(fill_value = 0): 결측지는 '0'으로 처리해주세요, 피벗 테이블 형성(총합계를 구하기 위함)
...
print(f"{daily_total_articles} 시리즈")

total_articles = daily_total_articles.sum()  
print(f"총합계 건수: {total_articles}")
...
print(pivot_df)
print(compressed_df)  # 시각화 진행해야하는 Table
...
# 그래프 디테일 설정
plt.title('날짜 구간별 상위 3개 카테고리 기사 비율 변화 (12개 구간 축약)', fontsize=16, pad=20)
