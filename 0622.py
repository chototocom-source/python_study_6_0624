...
df_clean = df[['일자', '통합 분류1']].dropna()  # '일자', '통합 분류1' 둘 중 하나라도 비어있는 값이 있으면 날리기!

date_category_counts = df_clean.groupby(['일자', '통합 분류1']).size()   # '일자'를 기준으로 '통합 분류1'에 있는 카데고리 값(개수)을 뽑아냄.
