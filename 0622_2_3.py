...
daily_total_articles = pivot_df.sum(axis=1) #날짜 기사합산 시리즈 -> key(일자) : value(기사 합산)
...
current_group_start = None  # 결합된_맨앞_날짜
current_sum = 0  # 합산된_기사수

print("=== [콘솔 확인] 날짜 결합 및 시리즈 생성 진행 ===")

for date, count in daily_total_articles.items():    # items(): 딕셔너리 key, value를 분리 시켜줌
    # 새로운 그룹의 시작점 지정
    if current_group_start is None:   # 결합된_맨앞_날짜: 선택적인 저장
        current_group_start = date
        
    current_sum += count  # 합산된_기사수: 누적
    
    # 더하고 있는 값이 평균보다 커지면, 만약 마지막의 기사수가 평균보다 작은 경우는 
    # 아래 if식으로 처리하고 있음
    if current_sum >= average_daily_articles:
        combined_data[current_group_start] = current_sum  # 딕셔너리로 저장함.
